...
import org.silva.settlement.core.chain.gateway.http.tls.TLS;
...
public final class SelfSignedP12Certificate {
  ...
  public SSLContext getCustomSslContext()
    ...
    var trustManagerFactory = getTrustManagerFactory();
    var keyManagerFactory = getKeyManagerFactory();
    ...
    try {
      final KeyStore trustStore =
              KeyStore.getInstance(
                      getTrustStoreFile().toFile(), getPassword());
  ...
  public Optional<KeyManagerFactory> getKeyManagerFactory() {
    ...
    try {
      final char[] keyStorePassword = getPassword();
      final KeyStore keyStore =
              KeyStore.getInstance(getKeyStoreFile().toFile(), keyStorePassword);
