...
public class QuorumCert extends Persistable {
    ...
    public static QuorumCert certificateForGenesisFromLedgerInfo(LedgerInfo ledgerInfo, byte[] genesisId) {
        EventInfo ancestor = EventInfo.build(ledgerInfo.getEpoch() + 1, 0, genesisId, ledgerInfo.getExecutedStateId(), ledgerInfo.getNumber(), ledgerInfo.getTimestamp(), EpochStateHolder.EMPTY_EPOCH_STATE_HOLDER, EpochStateHolder.EMPTY_EPOCH_STATE_HOLDER);
    ...
    public boolean isEpochChange() {
        return this.ledgerInfoWithSignatures.getLedgerInfo().getNewCurrentEpochState().isPresent();
    }
