...
import static org.silva.settlement.infrastructure.bytes.ByteUtil.intToBytes;
import static org.silva.settlement.infrastructure.bytes.ByteUtil.longToBytes;
import static org.silva.settlement.infrastructure.bytes.ByteUtil.numberOfLeadingZeros;
...
// todo for pool
public final class DataWord extends Persistable implements Comparable<DataWord> {
    ...
    public static DataWord buildForEmpty() {
        return new DataWord();
    }
    ...
    public static DataWord ofQuick(byte[] data) {
    ...
    public static DataWord of(byte[] data) {
        ...
        if (data.length == 32)
            ...
            throw new RuntimeException(String.format("Data word can't exceed 32 bytes: 0x%s", toHexString(data)));
        }
    }
